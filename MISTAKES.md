...
### 2026-07-15
**What happened:** During a large a11y/security/perf pass I ran `prettier --write "src/components/**/*.tsx" "src/app/**/*.tsx"` across the *entire* tree, which reordered Tailwind classes in ~50 files I never functionally touched — bloating the diff to 139 files. To clean it up I wrote a marker-grep heuristic to classify files as "functional" vs "formatting-only" and `git checkout master --`'d the "formatting-only" set. The heuristic's markers (`role=`, `-dark`, `aria-`, etc.) missed logic-only changes, so the revert silently clobbered 5 real changes (two P2 query-scoping fixes, the transcribe MIME gate, the members owner-demotion guard, and 2 SSRF tests). Build + tests stayed green because none of those 5 had direct test coverage, which nearly hid the regression.
**Why it was wrong:** Two stacked errors: (1) formatting the whole tree instead of only the files I edited violated "don't touch code I didn't ask you to" and created huge review noise; (2) using a fuzzy grep heuristic to drive a *destructive* bulk `git checkout` — reverting real work based on whether a diff happened to contain certain tokens. "Tests still pass" masked it because the clobbered changes were logic-only with no tests.
**Rule:** Only run the formatter on the specific files you changed (`prettier --write <those paths>`), never a broad glob over the tree. Never drive a destructive bulk `git checkout`/revert from a fuzzy content heuristic — to unwind formatting noise, restore files by an *explicit all/deny list you can eyeball*, or `git stash` + reapply, and diff-review before and after. After any bulk revert, re-verify by listing the exact changes expected and grepping each is still present — don't trust green tests, since logic-only changes often have none.

...
| DB migration | ✅ Done | `db:push` applied to Neon 2026-07-06 (twice — spaces FK change too) (`drizzle-kit` needs `DATABASE_URL` passed explicitly — it doesn't read `.env.local` on its own) — all tables/indexes through the FTS index are live |
